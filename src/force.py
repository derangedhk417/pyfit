...
class TorchForceCalculator:
	...
	# This will displace all neighbor displacement vectors along
	# the specified axis by the specified amount. This is intended
	# for use as a means of calculating finite difference gradients.
	def displace(self, axis, displacement):
		if not self.neighbors_loaded:
			msg = "You need to call loadNeighbors before you can call this."
			raise Exception(msg)

		self.neighbor_l[:, :, axis] -= displacement
		self.neighbor_r[:, :, axis] -= displacement
		self.dupl[:, :, axis]       -= displacement
